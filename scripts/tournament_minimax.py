...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a minimax tournament across all teams.")
    ...
    parser.add_argument("--search-actions", type=int, default=14)
    parser.add_argument("--top", type=int, default=10)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/minimax_tournament.json"),
...
def main() -> None:
    ...
    print(
        ...
        f"matches_per_pair={args.matches_per_pair} depth={args.depth}"
    )
...
def simulate_match(
    ...
    config = MinimaxConfig(depth=depth, max_actions=search_actions)
    agents = {0: MinimaxAgent(config), 1: MinimaxAgent(config)}
